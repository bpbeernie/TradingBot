    ...
    def on_realtime_update(self, reqId, time, open_, high, low, close, volume, wap, count):
        ...
        if not self.startingBars or len(self.startingBars) < 12:
            ...
            if self.symbol not in gb.Globals.getInstance().activeOrders:
                ...
                self.profitTargetForLong = expectedHigh + const.EXTRACASH / self.quantity
                self.profitTargetForShort = expectedLow - const.EXTRACASH / self.quantity
                self.stopLossForLong = expectedLow - risk
                self.stopLossForShort = expectedHigh + risk

            
                if high > expectedHigh and not self.executionTracker.isShortOrderExecuted():                    
                ...
                elif low < expectedLow and not self.executionTracker.isLongOrderExecuted():
